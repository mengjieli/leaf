namespace leaf {
    ...
    export class ScrollerShaderTask extends Shader {
        ...
        private offxs: number[] = [];
        private offys: number[] = [];
        ...
        addTask(texture: Texture, matrix: ecs.Matrix, alpha: number, blendMode: BlendMode, tint: number, offx: number = 0, offy: number) {
            if (texture.dirty) {
                texture.update();
            }
        ...
        private static _shader: ScrollerShaderTask;

        static get shader(): ScrollerShaderTask {
            if (!this._shader) {
                this._shader = new ScrollerShaderTask() as any;
            }
            return this._shader;
        }
