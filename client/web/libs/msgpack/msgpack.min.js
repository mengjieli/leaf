(function msgpack() {
  function serializeMsgPack(data) {
      ...
      switch (typeof data) {
        ...
        case "object":
          if (Array.isArray(data))
            ...
            if (data) {
              if (objs.has(data)) {
                throw 'msgpack 重复打包的对象:' + JSON.stringify(old)
              }
              objs.add(data);
            }
            appendObject(data);
    ...
    function appendBin(data) {
      ...
      if (len < 0xFF) {
        ...
        appendByte(0xc6);
        appendByte((len >> 24) & 0xFF);
        appendByte((len >> 16) & 0xFF);
        appendByte((len >> 8) & 0xFF);
        appendByte(len & 0xFF);
        appendBytes(data);
      }
      //       +--------+--------+========+
  ...
  function deserializeMsgPack(array) {
    if(array instanceof Array) array = new Buffer(array);
    "use strict";
    var pow32 = 0x100000000;   // 2^32
    var pos = 0;
    var data = read();
    ...
    function read() {
      ...
      if (byte === 0xc4) return readBin2(1);   // bin 8
      if (byte === 0xc5) return readBin2(2);   // bin 16
      if (byte === 0xc6) return readBin2(4);   // bin 32
    ...
    function readBin2(size) {
      var len;
      if (size == 1) {
        len = array[pos];
      } else if (size == 2) {
        len = array[pos] << 8 | array[pos + 1];
      } else if (size == 4) {
        len = array[pos] << 24 | array[pos + 1] << 16 | array[pos + 2] << 8 | array[pos + 3];
      }
      pos += size;
      var data = array.subarray(pos, pos + len);
      pos += len;
      return data;
    }
  ...
  msgpack.encode = serializeMsgPack;
  msgpack.decode = deserializeMsgPack;
  try {
