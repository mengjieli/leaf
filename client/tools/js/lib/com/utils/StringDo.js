...
StringDo.isNumber = function (char) {
    var code = char.charCodeAt(0);
    return code >= "0".charCodeAt(0) && code <= "9".charCodeAt(0) ? true : false;
}

StringDo.isId = function (str) {
    if (!StringDo.isCharABC(str[0])) return false;
    for (let i = 1; i < str.length; i++) {
        if (!StringDo.isCharABC(str[i]) && !StringDo.isNumber(str[i])) return false;
    }
    return true;
}
...
StringDo.reverse = function (str) {
    return str.split("").reverse().join("");
}

StringDo.deleteNotes = function (content) {
    let pos = 0;
    while (true) {
        let start1 = content.slice(pos, content.length).search(new RegExp('//[^\r\n]*[\r\n]'));
        let start2 = content.slice(pos, content.length).search(new RegExp('/\\*'));
        if (start1 == -1 && start2 == -1) break;
        if (start1 != -1 && start2 != -1 && start1 < start2 || start1 != -1 && start2 == -1) {
            let noteLength = content.slice(pos + start1, content.length).search(new RegExp('[\r\n]'));
            content = content.slice(0, pos + start1) +
                content.slice(pos + start1 + noteLength, content.length);
            pos = start1;
        } else if (start1 != -1 && start2 != -1 && start1 > start2 || start1 == -1 && start2 != -1) {
            let noteLength = content.slice(pos + start2, content.length).search(new RegExp('\\*/')) + 2;
            content = content.slice(0, pos + start2) +
                content.slice(pos + start2 + noteLength, content.length);
            pos = start2;
        }
    }
    return content;
