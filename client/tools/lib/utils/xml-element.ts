...
import { XMLNameSpace } from './xml-name-space'

export class XMLElement extends XMLAttribute {
  namespaces;
  ...
  filter(filterName?: (elementName: string) => boolean, filterAttribute?: (attribute: XMLAttribute) => boolean, layer: number = -1): XMLElement[] {
    let list = [];
    this.filterInChildren(list, filterName, filterAttribute, layer);
    return list;
  }

  filterInChildren(list: XMLElement[], filterName?: (elementName: string) => boolean, filterAttribute?: (attribute: XMLAttribute) => boolean, layer = -1) {
  ...
  getAllElements(): XMLElement[] {
    var res = [this];
    for (var i = 0; i < this.list.length; i++) {
      res = res.concat(this.list[i].getAllElements() as any);
    }
    return res;
  }
