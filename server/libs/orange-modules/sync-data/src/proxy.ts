namespace syncData {
  ...
  export class Proxy extends orange.NetProxy implements orange.INetProxy {
    ...
    public set connection(c: orange.INetConnection) {
      this._connection = c;
      orange['$onAt'](this._connection, orange.Event.CLOSE, async (e) => {
        ...
        if (!this.quit) {
          ...
          if (data.reason == orange.ConnectionCloseReason.DISCONNECT || data.reason == orange.ConnectionCloseReason.HERT_TIMEOUT
            || data.reason == orange.ConnectionCloseReason.RECONNECT_ERROR && this.curReconnectCount < this.reconnectCount) {
            e.stop();
     ...
     */
    @orange.modify
    public receive(data: INetReceiveMessage): void {
      ...
      if (this.syncCommands) {
        ...
        if (this.syncAll || this.syncCommands.indexOf(data.command as string) != -1) {
      ...
      this.resolveAsyncMessage(data.sequence, { command: data.command, errorCode: data.errorCode, errorMessage: data.errorMessage, body: data.body } as INetBack);
      this.receiveMessage(data.command, data);
    }

    private records = {};

    public send(data: any): orange.INetSendMessage {
    ...
    public request(data: any, back?: (data: any) => void): Promise<any> {
