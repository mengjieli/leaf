...
var StartCompile = /** @class */ (function (_super) {
                        ...
                        if (url && url.slice(url.slice(url.length - 3, url.length)) != '.ts') {
                            ...
                            if (file.exists) {
                                ...
                                compiler.pushCompile(url, source, function (content) {
                                    if (_this.useModule)
                                        content = _this.replaceModuleFile(content, url);
                                    _this.send({
                                        "cmd": "OnCompile",
                                        "rSeq": 0,
                                        "code": 0,
    ...
    StartCompile.prototype.replaceModuleFile = function (file, fileURL) {
        fileURL = fileURL.slice(global["params"].rootPath.length, fileURL.length);
        var projectName = global["params"].rootPath.split("/")[global["params"].rootPath.split("/").length - 1];
        ...
        // console.log(finds);
        finds && finds.forEach(function (find) {
            // console.log(find);
            var oldFind = find;
            var name = find.match(/var [_a-zA-Z0-9_]+ =/)[0].slice(4, find.match(/var [_a-zA-Z0-9_]+ =/)[0].length - 2);
            var url = find.match(/require\("[./0-9a-zA-Z_\-]+"/)[0].slice("require(".length, find.match(/require\("[./0-9a-zA-Z_\-]+"/)[0].length);
            var oldURL = url;
            var requireFileName = url.split("/")[url.split("/").length - 1];
            // console.log("???!!!", requireFileName)
            requireFileName = requireFileName.slice(0, requireFileName.length - 1);
            var src = path_1.join(global["params"].rootPath, "src/");
            var files = (new lib.File(src)).readFilesWithEnd("ts");
            var findURL;
            // console.log(requireFileName)
            // console.log("~~~~", findURL)
            files.forEach(function (file) {
                var name = file.url.split("/")[file.url.split("/").length - 1];
                name = name.split(".")[0];
